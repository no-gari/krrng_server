...
class ClayfulProductClient:
    ...
    def list_catalog_products(self, **kwargs):
        try:
            options = {
                'query': {
                    'collection': kwargs.get('commerce/collection', 'any'),
                    'limit': 120
                }
            }
            response = self.product.list(options)
        ...
        try:
            options = {
                'query': {
                    ...
                    'brand': kwargs.get('commerce/brand', 'any')
                }
            }
            response = self.product.list(options)
            return response
    ...
    def count_products(self, **kwargs):
        try:
            options = {'query': {'collection': kwargs.get('commerce/collection', 'any'), 'brand': kwargs.get(
                'commerce/brand', 'any')}}
            response = self.product.count(options)
            return response
        except Exception as err:
